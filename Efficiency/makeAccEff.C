...
void oniaTree::AccEffCalc(string caseTag) {
	...
	if (!prAccFile_pbpb) {
		...
		if (isAcc && isPbPb && isPr) {
			cout << "[INFO] since the settings are good I will make it" << endl;
			AccCalc(caseTag);
			prAccFile_pbpb = TFile::Open(Form("FilesAccxEff%s/Acc/prAccHists_PP.root", caseTag.c_str()));
		} else {
			cout << "[ERROR] Please change your settings and retry." << endl;
			return;
		}
	}
...
void oniaTree::EffCalc(string caseTag) {
	...
	if (isAcc) {
		cout << "[ERROR] you're trying to make Efficiency with Acceptance trees." << endl;
		return;
	}

	int nptbinsAna = sizeof(ptbinsAna) / sizeof(double) - 1;

	TH1F* hdeno_pt = new TH1F("hdeno_pt", "N_{gen} vs p_{T}; p_{T}; N_{total}", nptbinsAna, ptbinsAna);
	hdeno_pt->Sumw2();
	TH1F* hnum_pt = new TH1F("hnum_pt", "Eff vs p_{T}; p_{T}; Eff", nptbinsAna, ptbinsAna);
	hnum_pt->Sumw2();
	if (caseTag.find("1DfinerBins") != std::string::npos) {
		hdeno_pt = new TH1F("hdeno_pt", "N_{gen} vs p_{T}; p_{T}; N_{total}", nptbins, ptbins);
		hdeno_pt->Sumw2();
		hnum_pt = new TH1F("hnum_pt", "Eff vs p_{T}; p_{T}; Eff", nptbins, ptbins);
		hnum_pt->Sumw2();
	}
	TH1F* hdeno_y = new TH1F("hdeno_y", "N_{gen} vs y; y; N_{total}", nybins, ybins);
	hdeno_y->Sumw2();
	TH1F* hnum_y = new TH1F("hnum_y", "Eff vs y; y; Eff", nybins, ybins);
	hnum_y->Sumw2();

	TH1F* hdeno_cent = new TH1F("hdeno_cent", "N_{gen} vs centrality; hiBin; N_{total}", 20, 0, 200);
	hdeno_cent->Sumw2();
	TH1F* hnum_cent = new TH1F("hnum_cent", "Eff vs centrality; hiBin; Eff", 20, 0, 200);
	hnum_cent->Sumw2();

	TH2F* hdeno_pty = new TH2F("hdeno_pty", "N_{gen} vs p_{T} and y; y; p_{T}; N_{total}", nybins, ybins, nptbins, ptbins);
	hdeno_pty->Sumw2();
	TH2F* hnum_noweights = new TH2F("hnum_noweights", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_noweights->Sumw2();
	TH2F* hnum_nominal = new TH2F("hnum_nominal", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_nominal->Sumw2();

	TH2F** hdeno_pty_cent = new TH2F*[ncentbins];
	TH2F** hnum_nominal_cent = new TH2F*[ncentbins];
	for (int i = 0; i < ncentbins; i++) {
		hdeno_pty_cent[i] = new TH2F(Form("hdeno_pty_cent%d%d", centbins[i], centbins[i + 1]), "N_{gen} vs p_{T} and y; y; p_{T}; N_{total}", nybins, ybins, nptbins, ptbins);
		hdeno_pty_cent[i]->Sumw2();
		hnum_nominal_cent[i] = new TH2F(Form("hnum_nominal_cent%d%d", centbins[i], centbins[i + 1]), "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
		hnum_nominal_cent[i]->Sumw2();
	}
	//for pp
	TH2F* hnum_muidtrg_plus1sig_syst = new TH2F("hnum_muidtrg_plus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_muidtrg_plus1sig_syst->Sumw2();
	TH2F* hnum_muidtrg_minus1sig_syst = new TH2F("hnum_muidtrg_minus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_muidtrg_minus1sig_syst->Sumw2();
	TH2F* hnum_muidtrg_plus1sig_stat = new TH2F("hnum_muidtrg_plus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_muidtrg_plus1sig_stat->Sumw2();
	TH2F* hnum_muidtrg_minus1sig_stat = new TH2F("hnum_muidtrg_minus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_muidtrg_minus1sig_stat->Sumw2();

	TH2F* hnum_glb_plus1sig_syst = new TH2F("hnum_glb_plus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_glb_plus1sig_syst->Sumw2();
	TH2F* hnum_glb_minus1sig_syst = new TH2F("hnum_glb_minus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_glb_minus1sig_syst->Sumw2();
	TH2F* hnum_glb_plus1sig_stat = new TH2F("hnum_glb_plus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_glb_plus1sig_stat->Sumw2();
	TH2F* hnum_glb_minus1sig_stat = new TH2F("hnum_glb_minus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_glb_minus1sig_stat->Sumw2();

	//for PbPb
	TH2F* hnum_muid_plus1sig_syst = new TH2F("hnum_muid_plus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_muid_plus1sig_syst->Sumw2();
	TH2F* hnum_muid_minus1sig_syst = new TH2F("hnum_muid_minus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_muid_minus1sig_syst->Sumw2();
	TH2F* hnum_muid_plus1sig_stat = new TH2F("hnum_muid_plus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_muid_plus1sig_stat->Sumw2();
	TH2F* hnum_muid_minus1sig_stat = new TH2F("hnum_muid_minus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_muid_minus1sig_stat->Sumw2();

	TH2F* hnum_trg_plus1sig_syst = new TH2F("hnum_trg_plus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_trg_plus1sig_syst->Sumw2();
	TH2F* hnum_trg_minus1sig_syst = new TH2F("hnum_trg_minus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_trg_minus1sig_syst->Sumw2();
	TH2F* hnum_trg_plus1sig_stat = new TH2F("hnum_trg_plus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_trg_plus1sig_stat->Sumw2();
	TH2F* hnum_trg_minus1sig_stat = new TH2F("hnum_trg_minus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_trg_minus1sig_stat->Sumw2();

	TH2F* hnum_trk_plus1sig_syst = new TH2F("hnum_trk_plus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_trk_plus1sig_syst->Sumw2();
	TH2F* hnum_trk_minus1sig_syst = new TH2F("hnum_trk_minus1sig_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_trk_minus1sig_syst->Sumw2();
	TH2F* hnum_trk_plus1sig_stat = new TH2F("hnum_trk_plus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_trk_plus1sig_stat->Sumw2();
	TH2F* hnum_trk_minus1sig_stat = new TH2F("hnum_trk_minus1sig_stat", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_trk_minus1sig_stat->Sumw2();

	TH2F* hnum_tag_syst = new TH2F("hnum_tag_syst", "Eff vs p_{T} and y; y; p_{T}; Eff", nybins, ybins, nptbins, ptbins);
	hnum_tag_syst->Sumw2();
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if (!isPbPb && isPr) {
			...
			weight = Gen_weight * findNcoll(cent);
		else
			weight = 1.0;
		weight = weight * normF;
		...
		for (int iQQ = 0; iQQ < Gen_QQ_size; iQQ++) {
			...
			if (caseTag.find("withPtWeights") != std::string::npos) {
				if (abs(jpsi_rap) < 1.2 && jpsi_pt < 6.5)
					weight *= 1;
				else {
					if (abs(jpsi_rap) >= 0 && abs(jpsi_rap) < 0.6)
						curve = (TH1D*)wHistograms[0]->At(0);
					else if (abs(jpsi_rap) >= 0.6 && abs(jpsi_rap) < 1.2)
						curve = (TH1D*)wHistograms[1]->At(0);
					else if (abs(jpsi_rap) >= 1.2 && abs(jpsi_rap) < 1.8)
						curve = (TH1D*)wHistograms[2]->At(0);
					else if (abs(jpsi_rap) >= 1.8 && abs(jpsi_rap) < 2.4)
						curve = (TH1D*)wHistograms[3]->At(0);
			...
			if (isPbPb) {
				tnp_weight = tnp_weight_trk_pbpb(muplY, 0) * tnp_weight_muid_pbpb(muplPt, muplY, 0) * tnp_weight_trg_pbpb(muplPt, muplY, filterIdxMuPl, 0) * tnp_weight_trk_pbpb(mumiY, 0) * tnp_weight_muid_pbpb(mumiPt, mumiY, 0) * tnp_weight_trg_pbpb(mumiPt, mumiY, filterIdxMuMi, 0);
				...
				tnp_weight = tnp_weight_trk_pbpb(muplY, 99) * tnp_weight_muid_pbpb(muplPt, muplY, 99) * tnp_weight_trg_pbpb(muplPt, muplY, filterIdxMuPl, 99) * tnp_weight_trk_pbpb(mumiY, 99) * tnp_weight_muid_pbpb(mumiPt, mumiY, 99) * tnp_weight_trg_pbpb(mumiPt, mumiY, filterIdxMuMi, 99);
				hnum_tag_syst->Fill(reco_rap, reco_pt, tnp_weight * weight * ptW);

				tnp_weight = tnp_weight_trk_pbpb(muplY, 0) * tnp_weight_muid_pbpb(muplPt, muplY, -1) * tnp_weight_trg_pbpb(muplPt, muplY, filterIdxMuPl, 0) * tnp_weight_trk_pbpb(mumiY, 0) * tnp_weight_muid_pbpb(mumiPt, mumiY, -1) * tnp_weight_trg_pbpb(mumiPt, mumiY, filterIdxMuMi, 0);
				hnum_muid_plus1sig_syst->Fill(reco_rap, reco_pt, tnp_weight * weight * ptW);

				tnp_weight = tnp_weight_trk_pbpb(muplY, 0) * tnp_weight_muid_pbpb(muplPt, muplY, -2) * tnp_weight_trg_pbpb(muplPt, muplY, filterIdxMuPl, 0) * tnp_weight_trk_pbpb(mumiY, 0) * tnp_weight_muid_pbpb(mumiPt, mumiY, -2) * tnp_weight_trg_pbpb(mumiPt, mumiY, filterIdxMuMi, 0);
				...
				tnp_weight = tnp_weight_trk_pbpb(muplY, 0) * tnp_weight_muid_pbpb(muplPt, muplY, 0) * tnp_weight_trg_pbpb(muplPt, muplY, filterIdxMuPl, -1) * tnp_weight_trk_pbpb(mumiY, 0) * tnp_weight_muid_pbpb(mumiPt, mumiY, 0) * tnp_weight_trg_pbpb(mumiPt, mumiY, filterIdxMuMi, -1);
				hnum_trg_plus1sig_syst->Fill(reco_rap, reco_pt, tnp_weight * weight * ptW);

				tnp_weight = tnp_weight_trk_pbpb(muplY, 0) * tnp_weight_muid_pbpb(muplPt, muplY, 0) * tnp_weight_trg_pbpb(muplPt, muplY, filterIdxMuPl, -2) * tnp_weight_trk_pbpb(mumiY, 0) * tnp_weight_muid_pbpb(mumiPt, mumiY, 0) * tnp_weight_trg_pbpb(mumiPt, mumiY, filterIdxMuMi, -2);
				...
				tnp_weight = tnp_weight_trk_pbpb(muplY, -1) * tnp_weight_muid_pbpb(muplPt, muplY, 0) * tnp_weight_trg_pbpb(muplPt, muplY, filterIdxMuPl, 0) * tnp_weight_trk_pbpb(mumiY, -1) * tnp_weight_muid_pbpb(mumiPt, mumiY, 0) * tnp_weight_trg_pbpb(mumiPt, mumiY, filterIdxMuMi, 0);
				hnum_trk_plus1sig_syst->Fill(reco_rap, reco_pt, tnp_weight * weight * ptW);

				tnp_weight = tnp_weight_trk_pbpb(muplY, -2) * tnp_weight_muid_pbpb(muplPt, muplY, 0) * tnp_weight_trg_pbpb(muplPt, muplY, filterIdxMuPl, 0) * tnp_weight_trk_pbpb(mumiY, -2) * tnp_weight_muid_pbpb(mumiPt, mumiY, 0) * tnp_weight_trg_pbpb(mumiPt, mumiY, filterIdxMuMi, 0);
				...
				tnp_weight = std::get<0>(muplGlb) * std::get<0>(muplMuIdTrg) * std::get<0>(mumiGlb) * std::get<0>(mumiMuIdTrg);
				hnum_nominal->Fill(reco_rap, reco_pt, tnp_weight * weight * ptW);
				if (caseTag.find("noTnpWeights") != std::string::npos) {
					if (fabs(reco_rap) < 2.4)
						hnum_pt->Fill(reco_pt, weight * ptW);
				...
				tnp_weight = std::get<0>(muplGlb) * (std::get<0>(muplMuIdTrg) + std::get<2>(muplMuIdTrg)) * std::get<0>(mumiGlb) * (std::get<0>(mumiMuIdTrg) + std::get<2>(mumiMuIdTrg));
				...
				tnp_weight = std::get<0>(muplGlb) * (std::get<0>(muplMuIdTrg) + std::get<1>(muplMuIdTrg)) * std::get<0>(mumiGlb) * (std::get<0>(mumiMuIdTrg) + std::get<1>(mumiMuIdTrg));
				...
				tnp_weight = (std::get<0>(muplGlb) + std::get<2>(muplGlb)) * std::get<0>(muplMuIdTrg) * (std::get<0>(mumiGlb) + std::get<2>(mumiGlb)) * std::get<0>(mumiMuIdTrg);
				...
				tnp_weight = (std::get<0>(muplGlb) + std::get<1>(muplGlb)) * std::get<0>(muplMuIdTrg) * (std::get<0>(mumiGlb) + std::get<1>(mumiGlb)) * std::get<0>(mumiMuIdTrg);
...
void oniaTree::AccCalc(string caseTag) {
	...
	if (!isAcc) {
		cout << "[ERROR] you're trying to make Acceptance with Efficiency trees." << endl;
		return;
	}
	int nptbinsAna = sizeof(ptbinsAna) / sizeof(double) - 1;
	...
	TH1F* hdeno_pt = new TH1F("hdeno_pt", "N_{gen} vs p_{T}; p_{T}; N_{total}", nptbinsAna, ptbinsAna);
	hdeno_pt->Sumw2();
	TH1F* hnum_pt = new TH1F("hnum_pt", "Acc vs p_{T}; p_{T}; Acc", nptbinsAna, ptbinsAna);
	hnum_pt->Sumw2();
	if (caseTag.find("1DfinerBins") != std::string::npos) {
		hdeno_pt = new TH1F("hdeno_pt", "N_{gen} vs p_{T}; p_{T}; N_{total}", nptbins, ptbins);
		hdeno_pt->Sumw2();
		hnum_pt = new TH1F("hnum_pt", "Acc vs p_{T}; p_{T}; Acc", nptbins, ptbins);
		hnum_pt->Sumw2();
	}

	TH1F* hdeno_y = new TH1F("hdeno_y", "N_{gen} vs y; y; N_{total}", nybins, ybins);
	hdeno_y->Sumw2();
	TH1F* hnum_y = new TH1F("hnum_y", "Acc vs y; y; Acc", nybins, ybins);
	hnum_y->Sumw2();

	TH2F* hdeno_pty = new TH2F("hdeno_pty", "N_{gen} vs p_{T} and y; y; p_{T}; N_{total}", nybins, ybins, nptbins, ptbins);
	hdeno_pty->Sumw2();
	TH2F* hnum_nominal = new TH2F("hnum_nominal", "Acc vs p_{T} and y; y; p_{T}; Acc", nybins, ybins, nptbins, ptbins);
	hnum_nominal->Sumw2();
	...
	if (!onlyPlot) {
		...
		for (Long64_t jentry = 0; jentry < nentries; jentry++) {
			...
			if (isAcc)
				...
				Gen_weight = 1;
				if (pthat >= 15 && pthat < 25)
					Gen_weight = 0.0247699;
				else if (pthat >= 25 && pthat < 35)
					Gen_weight = 0.00311931;
				else if (pthat >= 35 && pthat < 45)
					Gen_weight = 0.000693027;
				else if (pthat >= 45)
					Gen_weight = 0.000212618;
			} else if (isPbPb) {
				cent = getMCHiBinFromhiHF(hiHF);
			...
			for (int iQQ = 0; iQQ < Gen_QQ_size; iQQ++) {
				...
				if (isAcc)
					...
					ptTotal->Fill(jpsi_pt, Gen_weight);
			}
...
void oniaTree::ClosureTestPtWeights() {
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if (!isPbPb && isPr) {
			...
			Gen_weight = Gen_weight * findNcoll(cent) * normF;
		else
			Gen_weight = 1;
		...
		for (int iQQ = 0; iQQ < Gen_QQ_size; iQQ++) {
			...
			//apply the acceptance cuts
			if (!areGenMuonsInAcceptance2019(iQQ)) continue;
			if (accNum->GetBinContent(accNum->FindBin(jpsi_rap, jpsi_pt)) < 0.0001)
				weight = Gen_weight;
			else
			...
			if (accNum_ptw->GetBinContent(accNum_ptw->FindBin(jpsi_rap, jpsi_pt)) < 0.0001)
				weight = Gen_weight;
			else
