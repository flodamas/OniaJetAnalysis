...
public :
   ...
   plotMCPars(const char* file, bool isRapBinning=false, bool isPtBinning=false, bool isCentBinning=false, bool isWMean=false);
   virtual ~plotMCPars();
...
#ifdef plotMCPars_cxx
plotMCPars::plotMCPars(const char* file, bool isRapBinning, bool isPtBinning, bool isCentBinning, bool isWMean) : fChain(0), saveName(""),isRBinning(false),isPBinning(false),isCBinning(false), wMean(false)
{
// if parameter tree is not specified (or zero), connect the file
  ...
  saveName = Form("%s/mcParams",sPath.Data());
  isRBinning = isRapBinning;
  isPBinning = isPtBinning;
  isCBinning = isCentBinning;
  wMean = isWMean;
...
plotMCPars::~plotMCPars()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t plotMCPars::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t plotMCPars::LoadTree(Long64_t entry)
{
...
void plotMCPars::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("zmin", &zmin, &b_zmin);
...
void plotMCPars::Show(Long64_t entry)
{
...
Int_t plotMCPars::Cut(Long64_t entry)
{
