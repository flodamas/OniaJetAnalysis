...
void fitter(
  ...
  */
	...
	if (doCtauErrPDF || fitTest) {
		inputFitDir["CTAUERR"] = "";
		inputInitialFilesDir["CTAUERR"] = "";
	}
	if ((fitMass && !fitCtau) || fitTest) {
		inputFitDir["MASS"] = "";
		inputInitialFilesDir["MASS"] = "";
	}
	if (fitCtauTrue || fitTest) {
		inputFitDir["CTAUTRUE"] = "";
		inputInitialFilesDir["CTAUTRUE"] = "";
	}
	if (fitCtauReco || fitTest) {
		inputFitDir["CTAURECO"] = "";
	...
	if ((fitCtau && fitData && incBkg && !incJpsi) || fitTest) {
		inputFitDir["CTAUSB"] = "";
  ...
  */
	...
	for (map<string, vector<string>>::iterator FileCollection = InputFileCollection.begin(); FileCollection != InputFileCollection.end(); ++FileCollection) {
		...
		if ((FILETAG.find("MC") != std::string::npos) && fitMC == true) {
			...
			if ((FILETAG.find("NOPR") != std::string::npos)) {
				if (!incNonPrompt) continue;
			} // If we find Non-Prompt MC, check if the user wants to include Non-Prompt
			else if ((FILETAG.find("PR") != std::string::npos) && !incPrompt)
	...
	if (Workspace.size() == 0) {
		cout << "[ERROR] No onia tree files were found matching the user's input settings!" << endl;
		return;
	}
	...
	for (uint j = 0; j < cutVectors.size(); j++) {
		...
		for (unsigned int i = 0; i < cutVectors[j].size(); i++) {
			...
			while ((soDSTAG = static_cast<TObjString*>(nextDSTAG.Next()))) {
				...
				} else if (DSTAG.Contains("DATA") && strcmp(applyCorr, ""))
					wsName = Form("%s_jetR%d_%s%s", DSTAG.Data(), (int)(jetR * 10), applyCorr, (applyJEC ? "_JEC" : ""));
				else
					wsName = DSTAG + Form("_jetR%d", (int)(jetR * 10)) + (applyJEC ? "_JEC" : "");
				if (Workspace.count(wsName.Data()) > 0) {
					...
					if (doSimulFit) {
						...
						if (DSTAG.Contains("MCJPSIPR")) {
							incJpsi = true;
							incPsi2S = false;
							incPrompt = true;
							incNonPrompt = false;
						}
						if (DSTAG.Contains("MCJPSINOPR")) {
							incJpsi = true;
							incPsi2S = false;
							incPrompt = false;
							incNonPrompt = true;
						}
						if (DSTAG.Contains("MCPSI2SPR")) {
							incJpsi = false;
							incPsi2S = true;
							incPrompt = true;
							incNonPrompt = false;
						}
						if (DSTAG.Contains("MCPSI2SNOPR")) {
							incJpsi = false;
							incPsi2S = true;
							incPrompt = false;
							incNonPrompt = true;
					...
					cout << "[ERROR] The workspace for " << wsName.Data() << " was not found!" << endl;
					return;
				}
			}
		}
...
bool addParameters(string InputFile, vector<struct KinCuts>& cutVector, vector<map<string, string>>& parIniVector, bool isPbPb, bool doConstrFit) {
	...
	if (cutVector.size() == 0) {
		for (vector<map<string, string>>::iterator row = data.begin(); row != data.end(); ++row) {
			...
			parIniVector.push_back(parIni);
		}
	} else {
		if (data.size() != cutVector.size()) {
			cout << "[ERROR] The initial parameters in file " << InputFile << " are not consistent with previous files!" << endl;
			return false;
		}
		for (unsigned int i = 0; i < data.size(); i++) {
			struct KinCuts cut;
			if (!setParameters(data.at(i), cut, parIniVector.at(i), isPbPb, doConstrFit)) { return false; };
			if (!isEqualKinCuts(cut, cutVector.at(i), isPbPb)) {
				cout << "[ERROR] The bins in file " << InputFile << " are not consistent with previous files!" << endl;
				return false;
			}
		}
	}
	...
	for (map<string, string>::iterator col = row.begin(); col != row.end(); ++col) {
		...
		if (label == "rap") {
			if (col->second == "" || col->second.find("-") == std::string::npos) {
				cout << "[ERROR] Input column 'rap' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "-", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'rap' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("-") == std::string::npos) {
				cout << "[ERROR] Input column 'pt' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "-", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'pt' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("-") == std::string::npos) {
				cout << "[ERROR] Input column 'zed' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "-", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'zed' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("-") == std::string::npos) {
				cout << "[ERROR] Input column 'jetpt' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "-", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'jetpt' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("-") == std::string::npos) {
				cout << "[ERROR] Input column 'jetrap' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "-", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'jetrap' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (isPbPb) {
				if (col->second == "" || col->second.find("-") == std::string::npos) {
					cout << "[ERROR] Input column 'cent' has invalid value: " << col->second << endl;
					return false;
				}
				std::vector<double> v;
				if (!parseString(col->second, "-", v)) { return false; }
				if (v.size() != 2) {
					cout << "[ERROR] Input column 'cent' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
					return false;
				}
			...
			if (col->second == "" || col->second.find("-") == std::string::npos) {
				cout << "[ERROR] Input column 'mass' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "-", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'mass' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("->") != std::string::npos) {
				cout << "[ERROR] Input column 'ctau' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "->", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'ctau' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("-") == std::string::npos) {
				cout << "[ERROR] Input column 'ctauErr' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "-", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'ctauErr' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("->") != std::string::npos) {
				cout << "[ERROR] Input column 'ctauTrue' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "->", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'ctauTrue' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("->") != std::string::npos) {
				cout << "[ERROR] Input column 'ctauRes' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "->", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'ctauRes' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("->") != std::string::npos) {
				cout << "[ERROR] Input column 'ctauNRes' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "->", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'ctauNRes' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "" || col->second.find("->") != std::string::npos) {
				cout << "[ERROR] Input column 'ctauN' has invalid value: " << col->second << endl;
				return false;
			}
			std::vector<double> v;
			if (!parseString(col->second, "->", v)) { return false; }
			if (v.size() != 2) {
				cout << "[ERROR] Input column 'ctauN' has incorrect number of values, it should have 2 values but has: " << v.size() << endl;
				return false;
			}
			...
			if (col->second == "") {
				cout << "[ERROR] Input column 'Model' has empty value" << endl;
				return false;
			}
			parIni[col->first] = col->second;
		} else {
			if (col->second != "") {
				...
				if ((value.find("[") == std::string::npos) || (value.find("]") == std::string::npos)) {
					...
					cout << "[ERROR] Either ']' or '[' are missing in the initial parameter values!" << endl;
					return false;
				} else {
				...
				if (v.size() > 3 || v.size() < 1) {
					cout << "[ERROR] Initial parameter " << col->first << " has incorrect number of values, it has: " << v.size() << endl;
					return false;
				}
				...
				if (v.size() == 1) {
					...
					if (doConstrFit)
						parIni[col->first] = Form("%s[ %.6f, %.6f, %.6f]", col->first.c_str(), v.at(0), v.at(0) - abs(v.at(0)), v.at(0) + abs(v.at(0)));
					else
...
bool parseString(string input, string delimiter, vector<double>& output) {
	...
	while (input != "") {
		...
		if (end != input) {
			...
			cout << "[ERROR] The conversion from string to double failed!";
			return false;
		}
		input = end;
...
bool parseFile(string FileName, vector<map<string, string>>& data) {
	...
	if (header.size() == 0) {
		cout << "[ERROR] The header is null!" << endl;
		return false;
	}
	...
	for (vector<string>::iterator rHeader = header.begin(); rHeader != header.end(); ++rHeader) {
		if (*rHeader == "") {
			cout << "[ERROR] A column has no label!" << endl;
			return false;
		}
	}
...
bool getInputFileNames(const string InputTrees, map<string, vector<string>>& InputFileCollection) {
	...
	for (vector<vector<string>>::iterator row = content.begin(); row != content.end(); ++row) {
		...
		if (row->at(0) != "" && row->at(1) == "") {
			cout << "[ERROR] There is an empty file name in your InputTrees.txt, please fix it" << endl;
			return false;
		}
		if (row->at(0) == "" && row->at(1) != "") {
			cout << "[ERROR] There is an empty file tag in your InputTrees.txt, please fix it" << endl;
			return false;
		}
		if (row->at(0) != "" && row->at(1) != "") {
...
bool readFile(string FileName, vector<vector<string>>& content, const int nCol, int nRow) {
	if (nCol == 0 || nRow == 0) {
		cout << "[WARNING] Ignoring content of File: " << FileName << endl;
		return true;
	}
	...
	if (subdirs) {
		...
		while ((subdir = (TSystemFile*)next())) {
			if (subdir->IsDirectory() && string(subdir->GetName()) != "." && string(subdir->GetName()) != "..") {
				...
				cout << "[INFO] Input subdirectory: " << dirname + subdir->GetName() + "/"
				     << " found!" << endl;
			}
		}
	}
...
bool checkSettings(bool fitData, bool fitMC, bool fitPbPb, bool fitPP, bool fitMass, bool fitCtau, bool fitCtauTrue, bool fitCtauReco, bool fitRes, bool doCtauErrPDF, bool incJpsi, bool incPsi2S, bool incBkg, bool incPrompt, bool incNonPrompt, double jetR, bool cutCtau, bool doConstrFit, bool doSimulFit, bool wantPureSMC, const char* applyCorr, bool applyJEC, bool setLogScale, bool zoomPsi, bool incSS, int numCores) {
	...
	if (!fitMass && !fitCtau && !fitCtauTrue && !doCtauErrPDF && !fitCtauReco && !fitRes) {
		cout << "[ERROR] At least one distribution has to be selected for fitting, please select either Mass, CtauTrue, CtauErr, Ctau, CtauN or CtauReco!" << endl;
		return false;
	}
	if (fitCtau && fitRes) {
		cout << "[ERROR] We can not fit Ctau and CtauRes at the same time, please fix your input settings!" << endl;
		return false;
	}
	if (doConstrFit && (fitCtau || fitCtauTrue || fitCtauReco || fitRes)) {
		cout << "[ERROR] Constrained fits only implemented for invariant mass!" << endl;
		return false;
	}
	if (doConstrFit && doSimulFit) {
		cout << "[ERROR] Constrained fits not implemented with simultaneous fits!" << endl;
		return false;
	}
	if (fitCtauTrue && fitData) {
		cout << "[ERROR] We can not fit the truth ctau distribution on data, please select only MC!" << endl;
		return false;
	}
	if (fitCtauTrue && !incNonPrompt) {
		cout << "[ERROR] The ctau truth distribution is only fitted on Non-Prompt MC, please include the Non-Prompt components!" << endl;
		return false;
	}
	if (!fitData && !fitMC) {
		cout << "[ERROR] At least one type of dataset has to be selected for fitting, please select either Data or MC!" << endl;
		return false;
	}
	if (!fitMass && doSimulFit) {
		cout << "[ERROR] Can't perform simultanoeus fit without fitting the mass spectrum, please fix your fitter settings!" << endl;
		return false;
	}
	if (fitCtau && (!incPrompt && !incNonPrompt)) {
		cout << "[ERROR] No ctau models were included, please include either Prompt or NonPrompt when fitting ctau!" << endl;
		return false;
	}
	if (fitMC && (!incPrompt && !incNonPrompt)) {
		cout << "[ERROR] No MC datasets were included, please include either Prompt or Non-Prompt MC!" << endl;
		return false;
	}
	if (!fitPbPb && !fitPP) {
		cout << "[ERROR] No datasets were selected, please select either PbPb or PP!" << endl;
		return false;
	}
	if (!incJpsi && !incPsi2S && !incBkg) {
		cout << "[ERROR] No models were included for fitting, please include either Jpsi, Psi2S or Bkg" << endl;
		return false;
	}
	if ((!fitData || fitMC) && (!incJpsi && !incPsi2S)) {
		cout << "[ERROR] We can't fit only background on MC, please include also either Jpsi or Psi2S!" << endl;
		return false;
	}
	...
	if (numCores <= 0 || numCores > 32) {
		cout << "[ERROR] Invalid number of cores: " << numCores << " , please select an interger number between 1 and 32." << endl;
		return false;
	}
	if (doSimulFit && (!fitPbPb || !fitPP)) {
		cout << "[ERROR] Can't perform simultaneous fit without both PP and PbPb, please fix your fitter settings!" << endl;
		return false;
	}
	if (doSimulFit && (!incPsi2S || !incJpsi)) {
		cout << "[ERROR] Simultaneous fits require both Psi2S and Jpsi, please fix your fitter settings!" << endl;
		return false;
	}
	if (doSimulFit && !fitData) {
		cout << "[ERROR] Can't perform simultaneous fits on MC, please fix your fitter settings!" << endl;
		return false;
	}
	...
	if (!(jetR == 0.3 || jetR == 0.4)) {
		cout << "[ERROR] R value do not correspond to any jet trees available. Please choose 0.3, 0.4, or 0.5" << endl;
		return false;
	}
