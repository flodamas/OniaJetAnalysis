...
void plotPt(const char* workDirName, const char* varname, int iplot, const char* collTag, bool plotErr, const char* DSTag, bool doSig) {
	...
	if (string(collTag) != "")
		tg = plotVar(tr, varname, theCats, xaxis, collTag, plotErr, doSig);
	else {
...
int markerstyle(int i) {
	if (i == 0)
		...
		return kOpenTriangleUp;
	else if (i == 10)
		return kOpenTriangleDown;
	else
		return markerstyle(i % 11);
}
