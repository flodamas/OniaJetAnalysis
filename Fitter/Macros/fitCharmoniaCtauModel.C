...
bool fitCharmoniaCtauModel(RooWorkspace& myws,                 // Local Workspace
	...
	if (DSTAG.find("MC") != std::string::npos) {
		if (incJpsi && incPsi2S) {
			cout << "[ERROR] We can only fit one type of signal using MC" << endl;
			return false;
		}
		isMC = true;
	}
	if (!isMC) { wantPureSMC = false; }
	if (useSPlot && ((incBkg && incJpsi) || (incBkg && incPsi2S) || (incPsi2S && incJpsi))) {
		cout << "[ERROR] Only one object can be fitted using sPlot!" << endl;
		return false;
	}
	...
	if (importDS) {
		...
		if (usePerEventError) {
			...
			if (foundFit) {
				cout << "[INFO] The ctauErr fit was found and I'll load the ctau Error range used." << endl;
			} else {
				cout << "[ERROR] The ctauErr fit was not found!" << endl;
				return false;
			}
	...
	if (importDS) {
		...
		numEntries = myws.data(dsName.c_str())->sumEntries();
		if (numEntries <= 0) { doFit = false; }
	} else if (doFit && !(myws.data(dsName.c_str()))) {
		cout << "[ERROR] No local dataset was found to perform the fit!" << endl;
		return false;
	}
	...
	if (usectauBkgTemplate) {
		...
		pdfNames.push_back(Form("pdfCTAUCOND_Bkg_%s", COLL.c_str()));
	...
	if (useSPlot) {
		...
		if (!compDS && (incJpsi || incPsi2S || incBkg)) {
			...
			if (!fitCharmoniaMassModel(myws, inputWorkspace, cut, parIni, opt, outputDir,
			                           ...
			                           setLogScale, incSS, zoomPsi, ibWidth, getMeanPT)) { return false; }
			...
			if (myws.pdf(Form("pdfMASS_Tot_%s", (isPbPb ? "PbPb" : "PP")))) {
				...
				cout << "[ERROR] Mass PDF was not found!" << endl;
				return false;
			}
			...
			for (auto obj : objs) {
				if (myws.var(Form("N_%s_%s", obj.c_str(), (isPbPb ? "PbPb" : "PP")))) myws.var(Form("N_%s_%s", obj.c_str(), (isPbPb ? "PbPb" : "PP")))->setMin(0.0);
			}
		}
		if (createSPLOTDS) {
			if (importDS && !myws.data(dsName2Fit.c_str())) {
				if (!createCtauDSUsingSPLOT(myws, dsName, parIni, cut, incJpsi, incPsi2S, incBkg)) {
					cout << "[ERROR] Creating the Ctau Templates using sPLOT failed" << endl;
					return false;
				}
			}
		}
	...
	if (!loadFitResult) {
		...
		if (usePerEventError) {
			...
			if (!fitCharmoniaCtauErrModel(myws, inputWorkspace, cut, parIni, opt, outputDir,
			                              ...
			                              setLogScale, incSS, binWidth)) { return false; }
		}
		...
		if ((fitSideBand || useSPlot) && !usectauBkgTemplate) {
			...
			if (outputDir.find("PrMC") != std::string::npos) DSTAG = Form("MCJPSIPR_%s", (isPbPb ? "PbPb" : "PP"));
			...
			if (!found && gSystem->AccessPathName(FileName.c_str()) && inputFitDir["CTAURES"] != "") {
				...
				if (outputDir.find("PrMC") != std::string::npos)
					plotLabel = Form("_CtauRes_%s_NoBkg_jetR%d%s%s", parIni[Form("Model_CtauRes_%s", COLL.c_str())].c_str(), (int)(jetR * 10), (strcmp(applyCorr, "") ? Form("_%s", applyCorr) : ""), (applyJEC ? "_JEC" : ""));
			...
			std::vector<std::string> objs = {"Bkg", "Jpsi", "Psi2S"};
			bool hasPromptPdf = false;
			for (auto obj : objs) {
				if (myws.pdf(Form("pdfCTAU_%sPR_%s", obj.c_str(), (isPbPb ? "PbPb" : "PP")))) {
					cout << "[INFO] Setting Prompt " << obj << " parameters to constant!" << endl;
					hasPromptPdf = true;
			...
			if (!hasPromptPdf) {
				cout << "[ERROR] Prompt Ctau PDF was not found!" << endl;
				return false;
			}
	...
	if (isPbPb) {
		if (parIni.count("Model_CtauRes_PbPb") > 0) {
			...
			if (model.PbPb.CtauRes == CtauModel(0)) {
				cout << "[ERROR] The ctau resolution model: " << parIni["Model_CtauRes_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] Ctau Resolution model for PbPb was not found in the initial parameters!" << endl;
			return false;
		}
	} else {
		if (parIni.count("Model_CtauRes_PP") > 0) {
			...
			if (model.PP.CtauRes == CtauModel(0)) {
				cout << "[ERROR] The ctau resolution model: " << parIni["Model_CtauRes_PP"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] Ctau Resolution model for PP was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (isPbPb && incBkg && incNonPrompt) {
		if (parIni.count("Model_BkgNoPR_PbPb") > 0) {
			...
			if (model.PbPb.Bkg.Ctau.NonPrompt == CtauModel(0)) {
				cout << "[ERROR] The background non-prompt ctau model: " << parIni["Model_BkgNoPR_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] Background non-prompt ctau model for PbPb was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (isPbPb && incBkg && incPrompt) {
		if (parIni.count("Model_BkgPR_PbPb") > 0) {
			...
			if (model.PbPb.Bkg.Ctau.Prompt == CtauModel(0)) {
				cout << "[ERROR] The background prompt ctau model: " << parIni["Model_BkgPR_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
	...
	if (isPbPb && incJpsi && incNonPrompt) {
		if (parIni.count("Model_JpsiNoPR_PbPb") > 0) {
			...
			if (model.PbPb.Jpsi.Ctau.NonPrompt == CtauModel(0)) {
				cout << "[ERROR] The Jpsi non-prompt ctau model: " << parIni["Model_JpsiNoPR_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] Jpsi non-prompt ctau model for PbPb was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (isPbPb && incJpsi && incPrompt) {
		if (parIni.count("Model_JpsiPR_PbPb") > 0) {
			...
			if (model.PbPb.Jpsi.Ctau.Prompt == CtauModel(0)) {
				cout << "[ERROR] The Jpsi prompt ctau model: " << parIni["Model_JpsiPR_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
	...
	if (isPbPb && incPsi2S && incNonPrompt) {
		if (parIni.count("Model_Psi2SNoPR_PbPb") > 0) {
			...
			if (model.PbPb.Psi2S.Ctau.NonPrompt == CtauModel(0)) {
				cout << "[ERROR] The psi(2S) non-prompt ctau model: " << parIni["Model_Psi2SNoPR_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] psi(2S) non-prompt ctau model for PbPb was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (isPbPb && incPsi2S && incPrompt) {
		if (parIni.count("Model_Psi2SPR_PbPb") > 0) {
			...
			if (model.PbPb.Psi2S.Ctau.Prompt == CtauModel(0)) {
				cout << "[ERROR] The psi(2S) prompt ctau model: " << parIni["Model_Psi2SPR_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
	...
	if (!isPbPb && incBkg && incNonPrompt) {
		if (parIni.count("Model_BkgNoPR_PP") > 0) {
			...
			if (model.PP.Bkg.Ctau.NonPrompt == CtauModel(0)) {
				cout << "[ERROR] The background non-prompt ctau model: " << parIni["Model_BkgNoPR_PP"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] Background non-prompt ctau model for PP was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (!isPbPb && incBkg && incPrompt) {
		if (parIni.count("Model_BkgPR_PP") > 0) {
			...
			if (model.PP.Bkg.Ctau.Prompt == CtauModel(0)) {
				cout << "[ERROR] The background prompt ctau model: " << parIni["Model_BkgPR_PP"] << " is invalid" << endl;
				return false;
			}
		} else {
	...
	if (!isPbPb && incJpsi && incNonPrompt) {
		if (parIni.count("Model_JpsiNoPR_PP") > 0) {
			...
			if (model.PP.Jpsi.Ctau.NonPrompt == CtauModel(0)) {
				cout << "[ERROR] The Jpsi non-prompt ctau model: " << parIni["Model_JpsiNoPR_PP"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] Jpsi non-prompt ctau model for PP was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (!isPbPb && incJpsi && incPrompt) {
		if (parIni.count("Model_JpsiPR_PP") > 0) {
			...
			if (model.PP.Jpsi.Ctau.Prompt == CtauModel(0)) {
				cout << "[ERROR] The Jpsi prompt ctau model: " << parIni["Model_JpsiPR_PP"] << " is invalid" << endl;
				return false;
			}
		} else {
	...
	if (!isPbPb && incPsi2S && incNonPrompt) {
		if (parIni.count("Model_Psi2SNoPR_PP") > 0) {
			...
			if (model.PP.Psi2S.Ctau.NonPrompt == CtauModel(0)) {
				cout << "[ERROR] The psi(2S) non-prompt ctau model: " << parIni["Model_Psi2SNoPR_PP"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] psi(2S) non-prompt ctau model for PP was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (!isPbPb && incPsi2S && incPrompt) {
		if (parIni.count("Model_Psi2SPR_PP") > 0) {
			...
			if (model.PP.Psi2S.Ctau.Prompt == CtauModel(0)) {
				cout << "[ERROR] The psi(2S) prompt ctau model: " << parIni["Model_Psi2SPR_PP"] << " is invalid" << endl;
				return false;
			}
		} else {
...
bool createCtauDSUsingSPLOT(RooWorkspace& ws, string dsName, map<string, string> parIni, struct KinCuts cut, bool incJpsi, bool incPsi2S, bool incBkg) {
	...
	if (!ws.data((dsName + "_SPLOT").c_str())) {
		...
		if (!isCompatibleDataset(*data, *(RooDataSet*)ws.data(dsName.c_str()))) {
			cout << "[ERROR] sPlot and original Datasets are inconsistent!" << endl;
			return false;
		}
		delete data;
		delete pdf;
