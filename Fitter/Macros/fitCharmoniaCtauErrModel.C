...
bool fitCharmoniaCtauErrModel(RooWorkspace& myws,                 // Local Workspace
	...
	if (incJpsi) {
		plotLabel = plotLabel + "_Jpsi";
		pdfNames.push_back(Form("%s_Jpsi_%s", pdfType.c_str(), COLL.c_str()));
	}
	if (incPsi2S) {
		plotLabel = plotLabel + "_Psi2S";
		pdfNames.push_back(Form("%s_Psi2S_%s", pdfType.c_str(), COLL.c_str()));
	}
	if (!isMC) {
		plotLabel = plotLabel + "_Bkg";
		pdfNames.push_back(Form("%s_Bkg_%s", pdfType.c_str(), COLL.c_str()));
	}
	...
	bool found = true;
	bool skipCtauErrPdf = !doCtauErrPdf;
	...
	if (importDS) {
		...
		if (numEntries <= 0) { doCtauErrPdf = false; }
	} else if (doCtauErrPdf && !(myws.data(dsName.c_str()))) {
		cout << "[ERROR] No local dataset was found to make the ctau Error Pdf!" << endl;
		return false;
	}
...
void setCtauErrGlobalParameterRange(RooWorkspace& myws, map<string, string>& parIni, struct KinCuts& cut, string label, double binWidth, bool useForCtauFits) {
	if (!useForCtauFits) {
		...
		getRange(hTot, (int)(ceil(2)), rangeErr); // KEEP THIS NUMBER WITH 2, JUST KEEP IT LIKE THAT :D
		hTot->Delete();
