...
bool buildCharmoniaCtauModel(RooWorkspace& ws, struct CharmModel model, map<string, string> parIni, string dsName,
	...
	if (incJpsi) {
		if (!defineCtauResolModel(ws, "Jpsi", model.CtauRes, parIni, isPbPb)) {
			cout << "[ERROR] Defining the Prompt Ctau Resolution Model failed" << endl;
			return false;
		}
		if (incPrompt) {
			if (!addSignalCtauModel(ws, "JpsiPR", model.Jpsi.Ctau.Prompt, parIni, isPbPb)) {
				cout << "[ERROR] Adding Prompt Jpsi Ctau Model failed" << endl;
				return false;
			}
		}
		if (incNonPrompt) {
			if (!addSignalCtauModel(ws, "JpsiNoPR", model.Jpsi.Ctau.NonPrompt, parIni, isPbPb)) {
				cout << "[ERROR] Adding NonPrompt Jpsi Ctau Model failed" << endl;
				return false;
			}
		}
	...
	if (incPsi2S) {
		if (!defineCtauResolModel(ws, "Psi2S", model.CtauRes, parIni, isPbPb)) {
			cout << "[ERROR] Defining the Prompt Ctau Resolution Model failed" << endl;
			return false;
		}
		if (incPrompt) {
			if (!addSignalCtauModel(ws, "Psi2SPR", model.Psi2S.Ctau.Prompt, parIni, isPbPb)) {
				cout << "[ERROR] Adding Prompt Psi2S Ctau Model failed" << endl;
				return false;
			}
		}
		if (incNonPrompt) {
			if (!addSignalCtauModel(ws, "Psi2SNoPR", model.Psi2S.Ctau.NonPrompt, parIni, isPbPb)) {
				cout << "[ERROR] Adding NonPrompt Psi2S Ctau Model failed" << endl;
				return false;
			}
		}
		if (incPrompt && !incNonPrompt) {
	...
	if (incBkg) {
		if (!usectauBkgTemplate) {
			if (!defineCtauResolModel(ws, "Bkg", model.CtauRes, parIni, isPbPb)) {
				cout << "[ERROR] Defining the Prompt Ctau Resolution Model failed" << endl;
				return false;
			}
			if (incPrompt) {
				if (!addBackgroundCtauModel(ws, "BkgPR", model.Bkg.Ctau.Prompt, parIni, isPbPb)) {
					cout << "[ERROR] Adding Prompt Bkg Ctau Model failed" << endl;
					return false;
				}
			}
			if (incNonPrompt) {
				if (!addBackgroundCtauModel(ws, "BkgNoPR", model.Bkg.Ctau.NonPrompt, parIni, isPbPb)) {
					cout << "[ERROR] Adding NonPrompt Bkg Ctau Model failed" << endl;
					return false;
				}
			}
			if (incPrompt && !incNonPrompt) {
			...
			if (usectauBkgTemplate) {
				if (incPrompt && incNonPrompt) {
					if (!createCtauBkgTemplate(ws, dsName, "pdfCTAUCOND", cut, incJpsi, incPsi2S, binWidth)) {
						cout << "[ERROR] Creating the Bkg Ctau Template failed" << endl;
						return false;
					}
				} else {
					cout << "[ERROR] To create the Bkg Ctau Template we need to activate prompt and nonpromt Jpsi" << endl;
					return false;
				}
			}
			if (!ws.pdf(Form("pdfCTAUERR_Bkg_%s", (isPbPb ? "PbPb" : "PP")))) { return false; }
	...
	switch (model) {
		case (CtauModel::SingleGaussianResolution):
			if (!(
				...
				cout << Form("[ERROR] Initial parameters where not found for Single Gaussian Ctau Resolution Model in %s", (isPbPb ? "PbPb" : "PP")) << endl;
				return false;
			}
			...
			cout << Form("[INFO] %s Single Gaussian Ctau Resolution PDF in %s included", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
			break;
		...
		case (CtauModel::DoubleGaussianResolution):
			...
			if (!(
				...
				cout << Form("[ERROR] Initial parameters where not found for Double Gaussian Ctau Resolution Model in %s", (isPbPb ? "PbPb" : "PP")) << endl;
				return false;
			}
			...
			cout << Form("[INFO] %s Double Gaussian Ctau Resolution PDF in %s included", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
			break;
		...
		case (CtauModel::TripleGaussianResolution):
			...
			if (!(
				...
				cout << Form("[ERROR] Initial parameters where not found for Triple Gaussian Ctau Resolution Model in %s", (isPbPb ? "PbPb" : "PP")) << endl;
				return false;
			}
			...
			cout << Form("[INFO] %s Triple Gaussian Ctau Resolution PDF in %s included", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
			break;
		...
		case (CtauModel::QuadrupleGaussianResolution):
			...
			cout << Form("[INFO] %s Quadruple Gaussian Ctau Resolution PDF in %s included", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
			break;

		default:

			cout << "[ERROR] Selected Ctau Resolution Model has not been implemented" << endl;
			return false;
	}
	...
	switch (model) {
		case (CtauModel::TripleDecay):
			if (!(
				...
				cout << Form("[ERROR] Initial parameters where not found for %s Background Triple Decay Ctau Model in %s", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
				return false;
			}
			...
			cout << Form("[INFO] %s Background Triple Decay Ctau PDF in %s included", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
			break;
		...
		case (CtauModel::QuadrupleDecay):
			if (!(
			      ...
			      parIni.count(Form("fDLIV_%s_%s", object.c_str(), (isPbPb ? "PbPb" : "PP"))))) {
				cout << Form("[ERROR] Initial parameters where not found for %s Background Quadruple Decay Ctau Model in %s", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
				return false;
			}
			...
			cout << Form("[INFO] %s Background Quadruple Decay Ctau PDF in %s included", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
			break;
		...
		case (CtauModel::Delta):
			...
			cout << Form("[INFO] %s Background Delta Ctau PDF in %s included", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
			break;

		default:

			cout << "[ERROR] Selected Background Ctau Model has not been implemented" << endl;
			return false;
	}
...
bool addSignalCtauModel(RooWorkspace& ws, string object, CtauModel model, map<string, string> parIni, bool isPbPb) {
	...
	switch (model) {
		case (CtauModel::SingleSidedDecay):
			...
			if (!(
			      parIni.count(Form("lambdaDSS_%s_%s", object.c_str(), (isPbPb ? "PbPb" : "PP"))))) {
				cout << Form("[ERROR] Initial parameters where not found for %s Signal Single Sided Decay Ctau Model in %s", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
				return false;
			}
			...
			cout << Form("[INFO] %s Signal Single Sided Decay Ctau PDF in %s included", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
			break;
		...
		case (CtauModel::Delta):
			...
			cout << Form("[INFO] %s Signal Delta Ctau PDF in %s included", object.c_str(), (isPbPb ? "PbPb" : "PP")) << endl;
			break;

		default:

			cout << "[ERROR] Selected Signal Ctau Model has not been implemented" << endl;
			return false;
	}
...
bool createCtauBkgTemplate(RooWorkspace& ws, string dsName, string pdfType, struct KinCuts cut, bool incJpsi, bool incPsi2S, double binWidth) {
	...
	RooDataHist* dataHist = new RooDataHist("tmp", "", *ws.var("ctau"), hTot);
	delete dataHist; // KEEP THIS LINE, IT IS A FIX FOR ROOKEYS
...
bool ctauBkgHistToPdf(RooWorkspace& ws, TH1D* hist, string pdfName, string dsName, vector<double> range, bool useDataSet) {
	...
	if (useDataSet) {
		if (ws.data(dsName.c_str()) == NULL) {
			cout << "[ERROR] DataSet " << dsName << " for Bkg Template was not found!" << endl;
			return false;
		}
		...
		for (int i = 0; i <= hist->GetNbinsX(); i++) {
			if (hist->GetBinContent(i) < 0) { hist->SetBinContent(i, 0.0000000001); }
		}
		...
		if (dataHist == NULL) {
			cout << "[ERROR] DataHist used to create " << pdfName << " failed!" << endl;
			return false;
		}
		if (dataHist->sumEntries() == 0) {
			cout << "[ERROR] DataHist used to create " << pdfName << " is empty!" << endl;
			return false;
		}
		if (fabs(dataHist->sumEntries() - hClean->GetSumOfWeights()) > 0.001) {
			cout << "[ERROR] DataHist used to create " << pdfName << "  "
			     << " is invalid!  " << endl;
			return false;
		}
		ws.import(*dataHist);
...
void findRangeFromPlot(vector<double>& range, struct KinCuts cut, double binWidth) {
	...
	for (int i = 1; i <= nBins; i++) {
		if ((ctauMin >= binLeftBoundLow) && (ctauMin < binBoundLeftHigh))
			range[0] = binLeftBoundLow;
		else {
			binLeftBoundLow += binWidth;
			binBoundLeftHigh += binWidth;
		}

		if ((ctauMax >= binRighttBoundLow) && (ctauMax < binBoundRightHigh))
			range[1] = binBoundRightHigh;
		else {
