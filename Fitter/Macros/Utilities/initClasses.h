...
struct ParticleMass {
	double JPsi, Psi2S, Y1S, Y2S, Y3S, Z;
};
...
T* clone(const T& s) {
	...
	if (!cloneSet) {
		cout << "[ERROR] Couldn't deep-clone " << s.GetName() << endl;
		return NULL;
	}
...
bool isCompatibleDataset(const RooDataSet& ds, const RooDataSet& ref, bool checkRange = true) {
	...
	if (ds.numEntries() != ref.numEntries()) {
		cout << "[ERROR] DataSets " << ds.GetName() << " and " << ref.GetName() << " don't have the same number of events!" << endl;
		return false;
	}
	if (ds.sumEntries() != ref.sumEntries()) {
		cout << "[ERROR] DataSets " << ds.GetName() << " and " << ref.GetName() << " don't have the same sum of weights!" << endl;
		return false;
	}
	...
	for (RooRealVar* it = (RooRealVar*)parIt->Next(); it != NULL; it = (RooRealVar*)parIt->Next()) {
		...
		if (ds.get()->find(it->GetName()) == NULL) {
			cout << "[ERROR] DataSet " << ds.GetName() << " does not contain the variable " << it->GetName() << " !" << endl;
			return false;
		}
		if (checkRange) {
			if (it->getMin() != ((RooRealVar*)ds.get()->find(it->GetName()))->getMin()) {
				cout << "[ERROR] " << it->GetName() << " Min Range disaggrement : "
				     << ds.GetName() << " ( " << ((RooRealVar*)ds.get()->find(it->GetName()))->getMin() << " ) "
				     << " and "
				     << ref.GetName() << " ( " << it->getMin() << " ) "
				     << " ! " << endl;
				return false;
			}
			if (it->getMax() != ((RooRealVar*)ds.get()->find(it->GetName()))->getMax()) {
				cout << "[ERROR] " << it->GetName() << " Max Range disaggrement : "
				     << ds.GetName() << " ( " << ((RooRealVar*)ds.get()->find(it->GetName()))->getMax() << " ) "
				     << " and "
				     << ref.GetName() << " ( " << it->getMax() << " ) "
				     << " ! " << endl;
				return false;
			}
		}
		if (ref.mean(*it) != ds.mean(*it)) {
			cout << "[ERROR] " << it->GetName() << " Mean Value disaggrement : "
			     << ds.GetName() << " ( " << ds.mean(*it) << " ) "
			     << " and "
			     << ref.GetName() << " ( " << ref.mean(*it) << " ) "
			     << " ! " << endl;
			return false;
		}
		if (ref.moment(*it, 2) != ds.moment(*it, 2)) {
			cout << "[ERROR] " << it->GetName() << " Variance Value disaggrement : "
			     << ds.GetName() << " ( " << ds.moment(*it, 2) << " ) "
			     << " and "
			     << ref.GetName() << " ( " << ref.moment(*it, 2) << " ) "
			     << " ! " << endl;
			return false;
		}
		if (ref.moment(*it, 3) != ds.moment(*it, 3)) {
			cout << "[ERROR] " << it->GetName() << " 3rd Moment Value disaggrement : "
			     << ds.GetName() << " ( " << ds.moment(*it, 3) << " ) "
			     << " and "
			     << ref.GetName() << " ( " << ref.moment(*it, 3) << " ) "
			     << " ! " << endl;
			return false;
		}
		if (ref.moment(*it, 4) != ds.moment(*it, 4)) {
			cout << "[ERROR] " << it->GetName() << " 4rd Moment Value disaggrement : "
			     << ds.GetName() << " ( " << ds.moment(*it, 4) << " ) "
			     << " and "
			     << ref.GetName() << " ( " << ref.moment(*it, 4) << " ) "
			     << " ! " << endl;
			return false;
		}
	}
	// DataSets are compatible if they passed all tests
	cout << "[INFO] DataSets " << ds.GetName() << " and " << ref.GetName() << " are compatible!" << endl;
	return true;
};
...
bool loadPreviousFitResult(RooWorkspace& myws, string FileName, string DSTAG, bool isPbPb, bool loadNumberOfEvents, bool updateN) {
	...
	for (RooRealVar* it = (RooRealVar*)parFunIt->Next(); it != NULL; it = (RooRealVar*)parFunIt->Next()) {
		...
		if (myws.var(name.c_str())) {
			...
			if (myws.var(reName.c_str())) {
				Double_t value = 0.0;
				if ((reName == Form("sigma1_Psi2S_%s", (isPbPb ? "PbPb" : "PP")))) {
					value = ws->var(name.c_str())->getValV() * MassRatio;
				} else if ((reName == Form("m_Psi2S_%s", (isPbPb ? "PbPb" : "PP")))) {
					value = ws->var(name.c_str())->getValV() * MassRatio;
				} else {
	...
	if (updateN && myws.pdf(Form("pdfMASS_Tot_%s", (isPbPb ? "PbPb" : "PP")))) {
		...
		if (!myws.data(dsNameCut.c_str())) dsNameCut = dsName;
		...
		if (myws.data(dsNameCut.c_str()) && ws->data(dsName.c_str()) && !isCompatibleDataset(*(RooDataSet*)myws.data(dsNameCut.c_str()), *(RooDataSet*)ws->data(dsName.c_str()), false)) {
			...
			std::map<std::string, double> dN_Old, dN_New, dNErr_Old;
			for (auto obj : objs) {
			...
			for (auto obj : objs) {
				if (myws.var(Form("N_%s_%s", obj.c_str(), (isPbPb ? "PbPb" : "PP")))) { dN_New[obj] = myws.var(Form("N_%s_%s", obj.c_str(), (isPbPb ? "PbPb" : "PP")))->getVal(); }
			}
			bool update = true;
			for (auto obj : objs) {
				if (dN_Old.count(obj) > 0 && ((dN_New.at(obj) - dN_Old.at(obj)) > dNErr_Old.at(obj))) { update = false; }
			}
			if (update) {
				for (auto obj : objs) {
				...
				for (auto obj : objs) {
					if (myws.var(Form("N_%s_%s", obj.c_str(), (isPbPb ? "PbPb" : "PP")))) { myws.var(Form("N_%s_%s", obj.c_str(), (isPbPb ? "PbPb" : "PP")))->setVal(dN_Old.at(obj)); }
				}
			...
			for (auto obj : objs) {
				if (myws.var(Form("N_%s_%s", obj.c_str(), (isPbPb ? "PbPb" : "PP")))) { myws.var(Form("N_%s_%s", obj.c_str(), (isPbPb ? "PbPb" : "PP")))->setError(dNErr_Old.at(obj)); }
			}
...
bool loadYields(RooWorkspace& myws, string FileName, string dsName, string pdfName) {
	...
	if (ws->data(dsName.c_str()) && myws.data(dsName.c_str())) {
		if (isCompatibleDataset(*(RooDataSet*)myws.data(dsName.c_str()), *(RooDataSet*)ws->data(dsName.c_str()))) {
			bool isPbPb = true;
			if (FileName.find("_PP_") != std::string::npos) isPbPb = false;
			...
			cout << "[INFO] RooDatasets used to extract the Yields are not compatible!" << endl;
			compDS = false;
		}
	} else {
		cout << "[INFO] RooDatasets used to extract the Yields were not found!" << endl;
		compDS = false;
	}
...
bool loadSPlotDS(RooWorkspace& myws, string FileName, string dsName) {
	...
	if (ws->data(dsName.c_str())) {
		...
		if (myws.data((dsName + "_INPUT").c_str())) {
			cout << "[INFO] RooDataset " << (dsName + "_INPUT") << " was imported!" << endl;
		} else {
			cout << "[ERROR] Importing RooDataset " << (dsName + "_INPUT") << " failed!" << endl;
		}
	} else {
...
int importDataset(RooWorkspace& myws, const RooWorkspace& inputWS, struct KinCuts cut, string label, bool cutSideBand = false) {
	...
	if (label.find("MC") != std::string::npos) {
		strCut = strCut + "&&" + indMuonCtauTrue + "&&" + indMuonCtauNRes + "&&" + indMuonCtauRes;
	} else {
		strCut = strCut + "&&" + indMuonCtauN;
	}
	...
	if (myws.data(Form("dOS_%s_SPLOT_INPUT", label.c_str()))) {
		...
		if (dataOS) {
			...
			if (dataOS->sumEntries() == 0) {
				cout << "[ERROR] No events from dataset " << Form("dOS_%s_SPLOT_INPUT", label.c_str()) << " passed the kinematic cuts!" << endl;
			} else if (!isCompatibleDataset(*dataOS, *(RooDataSet*)myws.data(Form("dOS_%s", label.c_str())))) {
				cout << "[ERROR] sPlot and Original Datasets are inconsistent!" << endl;
				delete dataOS;
				return -1;
			} else {
				myws.import(*dataOS, Rename(Form("dOS_%s_SPLOT", label.c_str())));
				if (myws.data(Form("dOS_%s_SPLOT", label.c_str()))) {
					cout << "[INFO] RooDataset " << Form("dOS_%s_SPLOT", label.c_str()) << " was imported!" << endl;
				} else {
					cout << "[ERROR] Importing RooDataset " << Form("dOS_%s_SPLOT", label.c_str()) << " failed!" << endl;
					delete dataOS;
					return -1;
				}
...
void getRange(TH1* hist, int nMaxBins, vector<double>& rangeErr) {
	...
	for (int i = binMaximum; i > 0; i--) {
		if (hist->GetBinContent(i) > 0.0) {
			if (binWithContent > 0 && ((binWithContent - i) > nMaxBins) && hist->GetBinContent(i) < 5.0) {
				firstBin = binWithContent;
				break;
			} else {
				binWithContent = i;
			}
	...
	for (int i = binMaximum; i < hist->GetNbinsX(); i++) {
		if (hist->GetBinContent(i) > 0.0) {
			if (binWithContent > 0 && ((i - binWithContent) > nMaxBins) && hist->GetBinContent(i) < 5.0) {
				lastBin = binWithContent + 1;
				break;
			} else {
				binWithContent = i;
			}
...
bool isSPlotDSAlreadyFound(RooWorkspace& myws, string FileName, vector<string> dsNames, bool loadDS) {
	...
	for (unsigned int i = 0; i < dsNames.size(); i++) {
		...
		if (!(ws->data(dsName.c_str()))) {
			cout << "[INFO] " << dsName << " was not found in: " << FileName << endl;
			found = false;
		}
		if (loadDS && found) {
			myws.import(*(ws->data(dsName.c_str())));
			if (myws.data(dsName.c_str())) {
				cout << "[INFO] sPlot DataSet " << dsName << " succesfully imported!" << endl;
			} else {
				cout << "[ERROR] sPlot DataSet " << dsName << " import failed!" << endl;
				found = false;
			}
		}
	}
...
bool isPdfAlreadyFound(RooWorkspace& myws, string FileName, vector<string> pdfNames, bool loadCtauErrPdf) {
	...
	for (unsigned int i = 0; i < pdfNames.size(); i++) {
		...
		if (!(ws->pdf(pdfName.c_str()))) {
			cout << "[INFO] " << pdfName << " was not found in: " << FileName << endl;
			found = false;
		}
		if (loadCtauErrPdf && found) {
			myws.import(*(ws->pdf(pdfName.c_str())));
			if (myws.pdf(pdfName.c_str())) {
				cout << "[INFO] Pdf " << pdfName << " succesfully imported!" << endl;
			} else {
				cout << "[ERROR] Pdf " << pdfName << " import failed!" << endl;
				found = false;
			}
