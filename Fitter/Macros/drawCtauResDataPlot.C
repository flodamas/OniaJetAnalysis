...
bool drawCtauResDataPlot(RooWorkspace& myws,         // Local workspace
	...
	vector<double> range;
	range.push_back(cut.dMuon.ctauN.Min);
	range.push_back(cut.dMuon.ctauN.Max);
	...
	if (outErr < 0) {
		cout << "[ERROR] Number of events is smaller after ctau cut: Total " << outTot << " and cutted " << (myws.data(dsNameCut.c_str())->numEntries()) << endl;
		return false;
	}
	...
	for (int i = 1; i < 5; i++) {
		if (myws.pdf(Form("%s%d_%s_%s", pdfType.c_str(), i, obj.c_str(), (isPbPb ? "PbPb" : "PP")))) {
			...
			myws.pdf(pdfTotName.c_str())->plotOn(frame, Name(Form("PDF%d", i)), Components(RooArgSet(*myws.pdf(Form("%s%d_%s_%s", pdfType.c_str(), i, obj.c_str(), (isPbPb ? "PbPb" : "PP"))))), Normalization(myws.data(dsNameCut.c_str())->sumEntries(), RooAbsReal::NumEvent), LineColor(COLOR[i - 1]), Precision(1e-5), NormRange("CtauNWindow"));
			nGauss++;
		}
	}

	myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("PDF"), Normalization(myws.data(dsNameCut.c_str())->sumEntries(), RooAbsReal::NumEvent), LineColor(kBlack), Precision(1e-5), NormRange("CtauNWindow"));
	...
	if (cut.dMuon.Zed.Max < 100) {
		t->DrawLatex(0.21, 0.86 - dy, Form("%.2f < z^{#mu#mu} #leq %.2f GeV/c", cut.dMuon.Zed.Min, cut.dMuon.Zed.Max));
		dy += 0.045;
	}
	...
	if (outErr > 0.0) {
		if (isPbPb) {
			t->DrawLatex(0.21, 0.86 - dy, Form("Excl: (%.4f%%) %.2f evts", (outErr * 100.0 / outTot), outErr));
			dy += 1.5 * 0.045;
		} else {
