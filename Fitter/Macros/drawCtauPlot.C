...
void drawCtauPlot(RooWorkspace& myws,         // Local workspace
	...
	if ((plotLabel.find("AccEff") != std::string::npos) || (plotLabel.find("_lJpsiEff") != std::string::npos))
		applyCorr = true;
	else
		applyCorr = false;
	...
	if (useSPlot && incBkg)
		dsOSName2Fit += "_BKG";
	else if (useSPlot && incJpsi)
		dsOSName2Fit += "_JPSI";
	else if (useSPlot && incPsi2S)
		dsOSName2Fit += "_PSI2S";
	...
	vector<double> range;
	range.push_back(cut.dMuon.ctau.Min);
	range.push_back(cut.dMuon.ctau.Max);

	double minRange = (double)(floor(range[0] * 10.) / 10.) + 0.1;
	double maxRange = (double)(ceil(range[1] * 10.) / 10.) + 0.1;
	if (!incNonPrompt) {
		if (abs(maxRange) > abs(minRange)) {
			minRange = -1.0 * abs(maxRange);
		} else {
	...
	if (myws.data(dsOSName2FitCut.c_str())) { normDSTot = myws.data(dsOSName2Fit.c_str())->sumEntries() / myws.data(dsOSName2FitCut.c_str())->sumEntries(); }
	double normJpsi = 1.0;
	if (myws.data(hOSNameJpsi.c_str())) { normJpsi = myws.data(dsOSName2Fit.c_str())->sumEntries() * normDSTot / myws.data(hOSNameJpsi.c_str())->sumEntries(); }
	double normPsi2S = 1.0;
	if (myws.data(hOSNamePsi2S.c_str())) { normPsi2S = myws.data(dsOSName2Fit.c_str())->sumEntries() * normDSTot / myws.data(hOSNamePsi2S.c_str())->sumEntries(); }
	double normBkg = 1.0;
	if (myws.data(hOSNameBkg.c_str())) { normBkg = myws.data(dsOSName2Fit.c_str())->sumEntries() * normDSTot / myws.data(hOSNameBkg.c_str())->sumEntries(); }
	double normTot = 1.0;
	if (myws.data(hOSName.c_str())) { normTot = myws.data(dsOSName2Fit.c_str())->sumEntries() * normDSTot / myws.data(hOSName.c_str())->sumEntries(); }
	...
	if (incBkg && (!incJpsi && !incPsi2S)) {
		if (!usectauBkgTemplate) {
			myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("BKG"), Normalization(normBkg, RooAbsReal::NumEvent), NumCPU(32), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameBkg.c_str()), kTRUE), FillStyle(1001), FillColor(kAzure - 9), VLines(), DrawOption("LCF"), Precision(1e-4));
			myws.data(dsOSName2Fit.c_str())->plotOn(frame, Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
			if (myws.pdf(Form("pdfCTAU_BkgPR_%s", (isPbPb ? "PbPb" : "PP")))) {
				myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("BKGPR"), Components(RooArgSet(*myws.pdf(Form("pdfCTAU_BkgPR_%s", (isPbPb ? "PbPb" : "PP"))))), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameBkg.c_str()), kTRUE), Normalization(normBkg, RooAbsReal::NumEvent), LineColor(kRed + 2), Precision(1e-4), NumCPU(32));
			}
			if (myws.pdf(Form("pdfCTAU_BkgNoPR_%s", (isPbPb ? "PbPb" : "PP")))) {
				myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("BKGNoPR"), Components(RooArgSet(*myws.pdf(Form("pdfCTAU_BkgNoPR_%s", (isPbPb ? "PbPb" : "PP"))))), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameBkg.c_str()), kTRUE), Normalization(normBkg, RooAbsReal::NumEvent), LineColor(kOrange + 10), Precision(1e-4), NumCPU(32));
			}
			myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("PDF"), Normalization(normBkg, RooAbsReal::NumEvent), NumCPU(32), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameBkg.c_str()), kTRUE), LineColor(kBlack), Precision(1e-4));
		} else {
			myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("PDF"), Normalization(numTot, RooAbsReal::NumEvent), NumCPU(32), LineColor(kBlack), Precision(1e-4));
		}
	}
	if (useSPlot) {
		if (incJpsi) {
			myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("JPSI"), Normalization(normJpsi, RooAbsReal::NumEvent), NumCPU(32), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameJpsi.c_str()), kTRUE), FillStyle(1001), FillColor(kAzure - 9), VLines(), DrawOption("LCF"), Precision(1e-4));
			myws.data(dsOSName2Fit.c_str())->plotOn(frame, Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
			if (myws.pdf(Form("pdfCTAU_JpsiPR_%s", (isPbPb ? "PbPb" : "PP")))) {
				myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("JPSIPR"), Components(RooArgSet(*myws.pdf(Form("pdfCTAU_JpsiPR_%s", (isPbPb ? "PbPb" : "PP"))))), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameJpsi.c_str()), kTRUE), Normalization(normJpsi, RooAbsReal::NumEvent), LineColor(kRed + 2), Precision(1e-4), NumCPU(32));
			}
			if (myws.pdf(Form("pdfCTAU_JpsiNoPR_%s", (isPbPb ? "PbPb" : "PP")))) {
				myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("BKGNoPR"), Components(RooArgSet(*myws.pdf(Form("pdfCTAU_JpsiNoPR_%s", (isPbPb ? "PbPb" : "PP"))))), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameJpsi.c_str()), kTRUE), Normalization(normJpsi, RooAbsReal::NumEvent), LineColor(kOrange + 10), Precision(1e-4), NumCPU(32));
			}
			myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("PDF"), Normalization(normJpsi, RooAbsReal::NumEvent), NumCPU(32), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameJpsi.c_str()), kTRUE), LineColor(kBlack), Precision(1e-4));
		}
		if (incPsi2S) {
			myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("PSI2S"), Normalization(normPsi2S, RooAbsReal::NumEvent), NumCPU(32), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNamePsi2S.c_str()), kTRUE), FillStyle(1001), FillColor(kAzure - 9), VLines(), DrawOption("LCF"), Precision(1e-4));
			myws.data(dsOSName2Fit.c_str())->plotOn(frame, Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
			if (myws.pdf(Form("pdfCTAU_Psi2SPR_%s", (isPbPb ? "PbPb" : "PP")))) {
				myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("JPSIPR"), Components(RooArgSet(*myws.pdf(Form("pdfCTAU_Psi2SPR_%s", (isPbPb ? "PbPb" : "PP"))))), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNamePsi2S.c_str()), kTRUE), Normalization(normPsi2S, RooAbsReal::NumEvent), LineColor(kRed + 2), Precision(1e-4), NumCPU(32));
			}
			if (myws.pdf(Form("pdfCTAU_Psi2SNoPR_%s", (isPbPb ? "PbPb" : "PP")))) {
				myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("BKGNoPR"), Components(RooArgSet(*myws.pdf(Form("pdfCTAU_Psi2SNoPR_%s", (isPbPb ? "PbPb" : "PP"))))), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNamePsi2S.c_str()), kTRUE), Normalization(normPsi2S, RooAbsReal::NumEvent), LineColor(kOrange + 10), Precision(1e-4), NumCPU(32));
			}
			myws.pdf(pdfTotName.c_str())->plotOn(frame, Name("PDF"), Normalization(normPsi2S, RooAbsReal::NumEvent), NumCPU(32), ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNamePsi2S.c_str()), kTRUE), LineColor(kBlack), Precision(1e-4));
		}
	}
	if (incSS) {
	...
	t->SetTextSize(0.03);
	t->DrawLatex(0.21, 0.86 - dy, "2018 HI Soft Muon ID");
	dy += 0.045;
	if (isPbPb) {
		t->DrawLatex(0.21, 0.86 - dy, "HLT_HIL3Mu0NHitQ10_L2Mu0_MAXdR3p5_M1to5_v1");
		dy += 0.045;
	} else {
		//t->DrawLatex(0.21, 0.86 - dy, "HLT_HIL1DoubleMuOpen_v1");
		dy += 0.045;
	}
	if (cut.dMuon.Zed.Max < 100) {
		t->DrawLatex(0.21, 0.86 - dy, Form("%.2f < z^{#mu#mu} #leq %2f", cut.dMuon.Zed.Min, cut.dMuon.Zed.Max));
		dy += 0.045;
	}
	t->DrawLatex(0.21, 0.86 - dy, Form("%.1f #leq p_{T}^{#mu#mu} < %.1f GeV/c", cut.dMuon.Pt.Min, cut.dMuon.Pt.Max));
	dy += 0.045;
	if (cut.jet.Pt.Max < 1000) {
		t->DrawLatex(0.21, 0.86 - dy, Form("%.1f #leq p_{T}^{jet} < %.1f GeV/c", cut.jet.Pt.Min, cut.jet.Pt.Max));
		dy += 0.045;
	}
	t->DrawLatex(0.21, 0.86 - dy, Form("%.1f #leq |y^{#mu#mu}| < %.1f", cut.dMuon.AbsRap.Min, cut.dMuon.AbsRap.Max));
	dy += 0.045;
	if (isPbPb) {
		t->DrawLatex(0.21, 0.86 - dy, Form("Cent. %d-%d%%", (int)(cut.Centrality.Start / 2), (int)(cut.Centrality.End / 2)));
		dy += 0.045;
	}
	if (outErr > 0.0) {
		if (isPbPb && isMC) {
			t->DrawLatex(0.21, 0.86 - dy, Form("Excl: (%.4f%%) %.4f evts", (outErr * 100.0 / outTot), outErr));
			dy += 1.5 * 0.045;
		} else {
			t->DrawLatex(0.21, 0.86 - dy, Form("Excl: (%.4f%%) %.0f evts", (outErr * 100.0 / outTot), outErr));
			dy += 1.5 * 0.045;
		}
	}
	...
	CMS_lumi(pad1, isPbPb ? 108 : 109, 33, "");
...
void printCtauParameters(RooWorkspace myws, TPad* Pad, bool isPbPb, string pdfName, bool isWeighted) {
	...
	for (RooRealVar* it = (RooRealVar*)parIt->Next(); it != NULL; it = (RooRealVar*)parIt->Next()) {
		...
		if (s2 == "CtauRes") {
			...
			s2 = "J/#psi[PR]";
		} else if (s2 == "Jpsi" && (s1 == "N" || s1 == "b")) {
			s2 = "J/#psi";
		} else if (s2 == "Psi2SNoPR") {
			s2 = "#psi(2S)[NoPR]";
		} else if (s2 == "Psi2SPR") {
			s2 = "#psi(2S)[PR]";
		} else if (s2 == "Psi2S" && (s1 == "N" || s1 == "b")) {
			s2 = "#psi(2S)";
		} else if (s2 == "BkgNoPR") {
			s2 = "bkg[NoPR]";
		} else if (s2 == "BkgPR") {
			s2 = "bkg[PR]";
		} else if (s2 == "Bkg" && (s1 == "N" || s1 == "b")) {
			s2 = "bkg";
		} else {
			continue;
		}
		...
		if (s1 == "N") {
			t->DrawLatex(0.69, 0.75 - dy, Form("%s = %.0f#pm%.0f ", label.c_str(), it->getValV(), it->getError()));
			dy += 0.045;
		} else if (s1 == "b") {
			t->DrawLatex(0.69, 0.75 - dy, Form("%s = %.4f#pm%.4f ", label.c_str(), it->getValV(), it->getError()));
			dy += 0.045;
		} else if (s1.find("sigma") != std::string::npos) {
			if (s2.find("Res") != std::string::npos) {
				t->DrawLatex(0.69, 0.75 - dy, Form("%s = %.2f#pm%.2f", label.c_str(), it->getValV(), it->getError()));
				dy += 0.045;
			} else {
				t->DrawLatex(0.69, 0.75 - dy, Form("%s = %.2f#pm%.2f mm", label.c_str(), it->getValV(), it->getError()));
				dy += 0.045;
			}
			...
			t->DrawLatex(0.69, 0.75 - dy, Form("%s = %.4f#pm%.4f mm", (label.insert(1, string("#"))).c_str(), it->getValV(), it->getError()));
			dy += 0.045;
		} else {
			t->DrawLatex(0.69, 0.75 - dy, Form("%s = %.4f#pm%.4f", label.c_str(), it->getValV(), it->getError()));
			dy += 0.045;
