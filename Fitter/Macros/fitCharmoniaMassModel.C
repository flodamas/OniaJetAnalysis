...
bool fitCharmoniaMassModel(RooWorkspace& myws,                 // Local Workspace
	...
	if (doSimulFit) {
		...
		simPdf->addPdf(*myws.pdf("pdfMASS_Tot_PbPb"), "PbPb");
		simPdf->addPdf(*myws.pdf("pdfMASS_Tot_PP"), "PP");
		myws.import(*simPdf);
		...
		if (loadFitResult) {
			if (loadPreviousFitResult(myws, FileName, DSTAG, false, (!isMC && !cutSideBand && loadFitResult == 1), loadFitResult == 1)) {
				skipFit = true;
			} else {
				skipFit = false;
			}
			if (loadPreviousFitResult(myws, FileName, DSTAG, true, (!isMC && !cutSideBand && loadFitResult == 1), loadFitResult == 1)) {
				skipFit = true;
			} else {
				skipFit = false;
			}
		...
		if (loadFitResult) {
			if (loadPreviousFitResult(myws, FileName, DSTAG, isPbPb, (!isMC && !cutSideBand && loadFitResult == 1), loadFitResult == 1)) {
				skipFit = true;
			} else {
				skipFit = false;
			}
...
void setCtauCuts(struct KinCuts& cut, bool isPbPb) {
	...
	//}
	cut.dMuon.ctauCut = "( ctau < (0.013 + (0.25/pt)) )";
	return;
};
	...
	if (isPbPb && incBkg) {
		if (parIni.count("Model_Bkg_PbPb") > 0) {
			...
			if (model.PbPb.Bkg.Mass == MassModel(0)) {
				cout << "[ERROR] The background model: " << parIni["Model_Bkg_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] Background mass model for PbPb was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (isPbPb && incJpsi) {
		if (parIni.count("Model_Jpsi_PbPb") > 0) {
			...
			if (model.PbPb.Jpsi.Mass == MassModel(0)) {
				cout << "[ERROR] The Jpsi model: " << parIni["Model_Jpsi_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] Jpsi mass model for PbPb was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (isPbPb && incPsi2S) {
		if (parIni.count("Model_Psi2S_PbPb") > 0) {
			...
			if (model.PbPb.Psi2S.Mass == MassModel(0)) {
				cout << "[ERROR] The psi2S model: " << parIni["Model_Psi2S_PbPb"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] psi(2S) mass model for PbPb was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (!isPbPb && incBkg) {
		if (parIni.count("Model_Bkg_PP") > 0) {
			...
			if (model.PP.Bkg.Mass == MassModel(0)) {
				cout << "[ERROR] The background model: " << parIni["Model_Bkg_PP"] << " is invalid" << endl;
				return false;
			}
		} else {
			cout << "[ERROR] Background mass model for PP was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (!isPbPb && incJpsi) {
		if (parIni.count("Model_Jpsi_PP") > 0) {
			...
			cout << "[ERROR] Jpsi mass model for PP was not found in the initial parameters!" << endl;
			return false;
		}
	}
	if (!isPbPb && incPsi2S) {
		if (parIni.count("Model_Psi2S_PP") > 0) {
			...
			cout << "[ERROR] psi(2S) mass model for PP was not found in the initial parameters!" << endl;
			return false;
		}
	}
