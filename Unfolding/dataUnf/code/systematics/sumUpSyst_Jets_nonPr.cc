...
void add(bool doPrompt = true, bool doMid = true){
  ...
  if(!doPrompt && doMid){
    ...
    outputfile = "../../unfOutput/totalSystErr/systErrs_NonPrompt_Mid_Jets.root";
  }
  ...
  if(!doPrompt && !doMid){
    ...
    outputfile = "../../unfOutput/totalSystErr/systErrs_NonPrompt_Fwd_Jets.root";
  }
  ...
  for(int i = 0; i < zBins; i++){
    ...
    finalBinErr = TMath::Sqrt(binErr_unfTrMatrix*binErr_unfTrMatrix+binErr_unfSI*binErr_unfSI+binErr_JES*binErr_JES+binErr_JER*binErr_JER);
