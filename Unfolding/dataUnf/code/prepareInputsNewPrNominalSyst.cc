...
void prepare(bool doPrompt = false, bool doMid = true, Int_t stepNumber = 1, double SF = 1.1){
  ...
  if(doPrompt){
    ...
    if(doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_midRapidity_49z15ptBins7zMeasBins_newNominal_syst%s.root",stepNumber,SF_name.c_str());
    if(!doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_fwdRapidity_50z15ptBins_newNominal_syst%s.root",stepNumber,SF_name.c_str());

    if(stepNumber > 1 && doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_Prompt_Mid_8iter_49z15ptBins7zMeasBins_newNominal_syst%s.root",stepNumber-1,SF_name.c_str());
    if(stepNumber > 1 && !doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_Prompt_Fwd_8iter_50z15ptBins_newNominal_syst%s.root",stepNumber-1,SF_name.c_str());
  }

  if(!doPrompt){
    filename = "../../TreesForUnfolding_corrAccEff/tree_MCJPSINOPR_PP_NoBkg_AccEff_JEC.root";

    if(doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_nonprompt_midRapidity_49z15ptBins7zMeasBins%s.root",stepNumber,SF_name.c_str());
    if(!doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_nonprompt_fwdRapidity_50z15ptBins%s.root",stepNumber,SF_name.c_str());

    if(stepNumber > 1 && doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_NonPrompt_Mid_8iter_49z15ptBins7zMeasBins%s.root",stepNumber-1,SF_name.c_str());
    if(stepNumber > 1 && !doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_NonPrompt_Fwd_8iter_50z15ptBins%s.root",stepNumber-1,SF_name.c_str());
  }
  ...
  */
  ...
  for(int nEv = 0; nEv < n_entries; nEv++){
    ...
    */
    ...
    double jt_pt_new_wJES = jt_pt_new*new_scaleFactor_JES;
...
void prepareInputsNewPrNominalSyst(Int_t step = 1, double SF = 1.1){
