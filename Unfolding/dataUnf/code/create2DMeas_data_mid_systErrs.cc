...
void create2DMeas_data_mid_systErrs(bool doPrompt = true){

  string filename1 = "";
  string filename2 = "";
  string filename3 = "";
  
  filename1 = "../data_results/unfoldingInput_lowJtPt_rap016_systErr.root";
  filename2 = "../data_results/unfoldingInput_midJtPt_rap016_systErr.root";
  filename3 = "../data_results/unfoldingInput_highJtPt_rap016_systErr.root";
  ...
  if(doPrompt){
    h_lowBin = (TH1F*)file1->Get("prHist_lowJtPt_rap016_systErr;1");
    h_centBin = (TH1F*)file2->Get("prHist_midJtPt_rap016_systErr;1");
    h_highBin = (TH1F*)file3->Get("prHist_highJtPt_rap016_systErr;1");
  }
  else{ 
    h_lowBin = (TH1F*)file1->Get("nprHist_lowJtPt_rap016_systErr;1");
    h_centBin = (TH1F*)file2->Get("nprHist_midJtPt_rap016_systErr;1");
    h_highBin = (TH1F*)file3->Get("nprHist_highJtPt_rap016_systErr;1");
  }
  
  TH2D *h_Meas = new TH2D("h_Meas","h_Meas",7,0.02,1.0,3,15.,45.);
  h_Meas->Sumw2();
  ...
  for(int icount=0; icount < 3; icount++){
    for(int jcount=0; jcount < 7; jcount++){
  ...
  can0->cd(1);
  h_lowBin->SetTitle("15 < jet p_{T} < 25");
  h_lowBin->Draw();

  can0->cd(2);
  h_centBin->SetTitle("25 < jet p_{T} < 35");
  h_centBin->Draw();

  can0->cd(3);
  h_highBin->SetTitle("35 < jet p_{T} < 45");
  h_highBin->Draw();

  if(doPrompt) can0->SaveAs("../plots/data_z_distr_promptMid_systErr.pdf");
  else can0->SaveAs("../plots/data_z_distr_nonpromptMis_systErr.pdf");

  TCanvas * can1 = new TCanvas ("can1","can1",600,600);
  can1->SetRightMargin(0.2);
  
  if(doPrompt) h_Meas->SetTitle("prompt mid-rapidity");
  else h_Meas->SetTitle("nonprompt mid-rapidity");
  ...
  if(doPrompt) can1->SaveAs("../plots/data_meas_promptMid_systErr.pdf");
  else can1->SaveAs("../plots/data_meas_nonpromptMid_systErr.pdf");

  
  string outputfile = "";
  if(doPrompt) outputfile = "../data_results/meas_data_prompt_mid_systErrs.root";
  else outputfile = "../data_results/meas_data_nonprompt_mid_systErrs.root";
