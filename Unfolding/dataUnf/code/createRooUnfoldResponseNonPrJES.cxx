...
void create(bool doPrompt = true, bool doMid = true, Int_t stepNumber = 1, double SF = 1.1){
  ...
  if(doPrompt && doMid) {
    inputName = Form("dataUnfNewMidBins/unfInput/step%i/unfolding_4D_prompt_midRapidity_49z15ptBins7zMeasBins_nonPrJES%s.root",stepNumber,SF_name.c_str());
  }
  if(doPrompt && !doMid) {
    inputName = Form("dataUnfNewMidBins/unfInput/step%i/unfolding_4D_prompt_fwdRapidity_50z15ptBins_nonPrJES%s.root",stepNumber,SF_name.c_str());
  }
  if(!doPrompt && doMid) {
...
void createRooUnfoldResponseNonPrJES(Int_t step = 1, double SF = 1.1){
