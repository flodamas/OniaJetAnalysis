...
void prepare(bool doPrompt = false, bool doMid = true, Int_t stepNumber = 1, double SF = 1.1){
  ...
  if(doPrompt){
    ...
    if(doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_midRapidity_49z15ptBins7zMeasBins_syst%s.root",stepNumber,SF_name.c_str());
    if(!doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_fwdRapidity_50z15ptBins_syst%s.root",stepNumber,SF_name.c_str());

    if(stepNumber > 1 && doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_Prompt_Mid_8iter_49z15ptBins7zMeasBins_syst%s.root",stepNumber-1,SF_name.c_str());
    if(stepNumber > 1 && !doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_Prompt_Fwd_8iter_50z15ptBins_syst%s.root",stepNumber-1,SF_name.c_str());
  }

  if(!doPrompt){
    filename = "../../TreesForUnfolding_corrAccEff/tree_MCJPSINOPR_PP_NoBkg_AccEff_JEC.root";

    if(doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_nonprompt_midRapidity_49z15ptBins7zMeasBins_syst%s.root",stepNumber,SF_name.c_str());
    if(!doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_nonprompt_fwdRapidity_50z15ptBins_syst%s.root",stepNumber,SF_name.c_str());

    if(stepNumber > 1 && doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_NonPrompt_Mid_8iter_49z15ptBins7zMeasBins_syst%s.root",stepNumber-1,SF_name.c_str());
    if(stepNumber > 1 && !doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_NonPrompt_Fwd_8iter_50z15ptBins_syst%s.root",stepNumber-1,SF_name.c_str());
  }
  ...
  */
  ...
  for(int nEv = 0; nEv < n_entries; nEv++){
    ...
    */
    ...
    if(jt_pt_new < 15.0 || jt_pt_new > 45.0) continue;
    if(jt_ref_pt < 15.0 || jt_ref_pt > 45.0) continue;
    if(TMath::Abs(jt_eta) > 2.4) continue;
    ...
    /*
    zWeight = 1.;
    if(doPrompt && doTrain) zWeight = getZWeight(doMid,gen_z2);
    if(!doPrompt && doTrain) zWeight = 1/getZWeight(doMid,gen_z2);
    */
    ...
    double jjt = (double)(TMath::Floor((jt_pt-15.)/10.));

    double iz = (double)(TMath::Floor(gen_z2*50.));
    if(doMid) iz = (double)(TMath::Floor((gen_z2-0.02)*49.));
    double jz = (double)(TMath::Floor(z2*5.));
    if(doMid) jz = (double)(TMath::Floor((z2-0.02)*7.));

    if(iz>49 && !doMid) iz=49;
    if(jz>4 && !doMid) jz=4;

    if(iz>48 && doMid) iz=48;
    if(jz>6 && doMid) jz=6;
    
    if(!doMid) h_trMatrix->Fill(5.*jjt+jz+0.5,50.*ijt+iz+0.5,finalCorr);
    else h_trMatrix->Fill(7.*jjt+jz+0.5,49.*ijt+iz+0.5,finalCorr);
...
void prepareInputsSyst(Int_t step = 1, double SF = 1.1){


  prepare(false,true,step,SF);
  prepare(false,false,step,SF);
