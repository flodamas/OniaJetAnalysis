...
void plot(bool doPrompt = true, bool doMid = false){
  ...
  if(doPrompt && doMid){
    can1->SaveAs("../plots/unf_data_prompt_mid_newNominal_wScale.pdf");
  }

  if(doPrompt && !doMid){
    can1->SaveAs("../plots/unf_data_prompt_fwd_newNominal_wScale.pdf");
  }
...
void PlotRatios_DataUnfolded_JEScheck(){
