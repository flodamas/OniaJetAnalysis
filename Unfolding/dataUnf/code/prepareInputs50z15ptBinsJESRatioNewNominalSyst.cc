...
void prepare(bool doPrompt = false, bool doMid = true, Int_t stepNumber = 1, double SF = 1.1){
  ...
  if(doPrompt){
    ...
    if(doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_midRapidity_50z15ptBins%s_JESratio_newNominal_syst.root",stepNumber,SF_name.c_str());
    if(!doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_fwdRapidity_50z15ptBins%s_JESratio_newNominal_syst.root",stepNumber,SF_name.c_str());

    if(stepNumber > 1 && doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_Prompt_Mid_8iter_50z15ptBins%s_JESratio_newNominal_syst.root",stepNumber-1,SF_name.c_str());
    if(stepNumber > 1 && !doMid) filenamePrevStep = Form("../unfOutput/step%i/UnfoldedDistributions_Prompt_Fwd_8iter_50z15ptBins%s_JESratio_newNominal_syst.root",stepNumber-1,SF_name.c_str());
  }
...
void prepareInputs50z15ptBinsJESRatioNewNominalSyst(Int_t step = 1, double SF = 1.1){
